              ...
              <div class="col-lg-12 mb-3">
                <div class="card shadow mb-4">
                  <div class="card-header py-3">
                    <div class="row">
                      <div class="col-6">
                        <h2 class="m-0 font-weight-bold sub-judul">List Situs</h2>
                      </div>
                    ...
                    <table id="listArtikel" class="table" style="width:100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>{{ $f->nama }}</td>
                          <td><td>{{ $f->user_id != null ? $f->user->nama : 'Admin Cagar Budaya' }}</td></td>
                          <td>
                          ...
                          <td>
                              ...
                              <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="{{ route('situs_detail', $f->slug) }}">View Artikel</a></li>
                                <li><a class="dropdown-item" href="{{ route('admin.situs.edit', $f->id) }}">Edit</a></li>
...
@section('js')
  <script>
    $(document).on('click', '.btn-hapus', function(e) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          window.location.href = `/admin/parasanganta/situs/delete/${id}`
          Swal.fire(
            'Deleted!',
    ...
    $(document).on('click', '.btn-vqr', function(e) {
      let id = $(this).attr("data-id");
      $($.ajax({
        url: `/admin/parasanganta/situs/qr/${id}`,
        success: function (response) {
          $('#modal .modal-body').html(atob(response.qrCode));
          $('#modal .modal-footer').find('a').attr('href', `/admin/parasanganta/konten/cetak/${response.slug}?cetak=situs`);
          $('#modal').modal('show');
        }
      }));
    });
