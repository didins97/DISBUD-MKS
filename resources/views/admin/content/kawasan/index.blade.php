              ...
              <div class="col-lg-12 mb-3">
                <div class="card shadow mb-4">
                  <div class="card-header py-3">
                    <div class="row">
                      <div class="col-6">
                        <h2 class="m-0 font-weight-bold sub-judul">List Kawasan</h2>
                      </div>
                      <div class="col-6 text-end">
                        <a href="{{ route('admin.kawasan.add') }}" class="btn btn-secondary">
                          <i class="fa fa-plus mr-1"></i> Tambah
                        </a>
                        {{-- <a href="{{ route('admin.konten.cetak') }}?cetak=Kawasan" class="btn btn-secondary bg-success">
                        ...
                        <tr>
                          ...
                          <td>
                              ...
                              <ul class="dropdown-menu">
                                <li><a class="dropdown-item" href="{{ route('kawasan_detail', $f->slug) }}">View Artikel</a></li>
                                <li><a class="dropdown-item" href="{{ route('admin.kawasan.edit', $f->id) }}">Edit</a></li>
...
@section('js')
  <script>
    $(document).on('click', '.btn-hapus', function(e) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          window.location.href = `/admin/parasanganta/kawasan/delete/${id}`
          Swal.fire(
            'Deleted!',
    ...
    $(document).on('click', '.btn-vqr', function(e) {
      let id = $(this).attr("data-id");
      $($.ajax({
        url: `/admin/parasanganta/kawasan/qr/${id}`,
        success: function (response) {
          $('#modal .modal-body').html(atob(response.qrCode));
          $('#modal .modal-footer').find('a').attr('href', `/admin/parasanganta/konten/cetak/${response.slug}?cetak=kawasan`);
          $('#modal').modal('show');
        }
      }));
    });
